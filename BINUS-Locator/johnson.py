...
def Dijkstra(graph, modifiedGraph, src, distance, index, path):
...
def JohnsonAlgorithm(graph, distance, path):
    count = 0
    edges = []
    ...
    # for i in modifiedGraph:
    #     print(i)

    for src in range(len(graph)):
        # print('\nShortest Distance with vertex ' +
              # str(src) + ' as the source:\n')
        Dijkstra(graph, modifiedGraph, src, distance, count, path)
        count += 1
